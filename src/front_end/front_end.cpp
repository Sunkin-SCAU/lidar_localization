...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    static Eigen::Matrix4f last_pose = init_pose_;//这里的init_pose为gps的位姿
...
bool FrontEnd::UpdateWithNewFrame(const Frame& new_key_frame) {
    ...
    // 更新局部地图
    local_map_frames_.push_back(key_frame);//如果局部地图大于20帧，那么将头部关键帧弹出
    ...
    for (size_t i = 0; i < local_map_frames_.size(); ++i) {
        ...
        *local_map_ptr_ += *transformed_cloud_ptr;//因为pcl重载了+=操作符，所以可以使用这种方法将他们拼合
    }
    has_new_local_map_ = true;

    // 更新ndt匹配的目标点云---------------检查局部地图的数量，如果少了就不进行滤波了
